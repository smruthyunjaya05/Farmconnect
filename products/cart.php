            ...
            <div class="container">
                <div class="row">
                    ...
                    <div class="col text-right">
                        ...
                        <?php if(count($allProducts) > 0): ?>
                        <button type="submit" name="submit" href="checkout.html" class="btn btn-lg btn-primary">Checkout <i class="fa fa-long-arrow-right"></i></button>
                        <?php endif; ?>
                        </form>
                    </div>
                </div>
    ...
    <script>
        $(document).ready(function() {
            ...
            $(".pro_qty").mouseup(function () {
                ...
                var pro_price = $el.find(".pro_price").text().replace('₹ ', '').trim(); // Remove the currency symbol and trim whitespace

                var subtotal = pro_qty * pro_price;
                $el.find(".subtotal_price").html("");        
                $el.find(".subtotal_price").append('₹ '+ subtotal); // Append the currency symbol after the total
            ...
            function fetch() {
                setInterval(function () {
                    ...
                    $('.subtotal_price').each(function() {
                        var subtotalText = $(this).text().replace('₹ ', '').trim(); // Remove the currency symbol and trim whitespace
                        var subtotal = parseFloat(subtotalText);
                        if (!isNaN(subtotal)) {
                            sum += subtotal;
                        }
                    });
                    $(".full_price").html('Total Price ₹ ' + sum.toFixed(2)); // Format the total price to 2 decimal places
